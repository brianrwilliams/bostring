...
\owen{We refer to this moduli space several times in the text, so perhaps we should give it a notation. Let's discuss what to use.}
...
The Grothendieck-Riemann-Roch theorem states that for a coherent sheaf $\cF$ on $C$, 
the Chern character $\ch(R\pi_* \cF)$ of its derived pushforward $R\pi_* \cF$  is given by 
\def\Td{{\rm Td}}
\begin{align*}
\pi_*( \ch(\cF) \Td(\cT_\pi)) &= \pi_*( (1 + c_1(\cF) + \frac{1}{2}(c_1(\cF)^2 - 2c_2(\cF)) + \cdots)(1 +\frac{1}{2}c_1(\cT_\pi)))\\
%&=  (1 + \pi^* c_1(\cT_X) + \pi^* \ch_2(\cT_X) + \cdots)(1 +\frac{1}{2}c_1(\cT_\pi)),
\end{align*}
where $\cT_\pi$ denotes the relative tangent bundle,
which is here just the tangent line bundle of a Riemann surface.
The first Chern class is the component of cohomological degree 2, namely
\[
(1-g) c_1(\cF) + \ch_2(\cF) 
\]
where $g$ denotes the genus of $C$.
For us $\cF$ is always a line bundle and thus $\ch_2(\cF) = 0$.
When $\cF = \sO_C \otimes V$, we get $(\dim V) (1-g)$, and
when $\cF = \cT_{\pi}$, we get $(1-g) (3g+3)$.
Hence, for the determinant line of global observables $H^*\left(\Obs^\q_{free}(C)\right)$ as a bundle over $C$,
we get
\[
c_1\left(H^*\left(\Obs^\q_{free}(\Sigma)\right)\right) = (g-1)(3g+3 - \dim(V)) =
\]
\owen{I am fucking this up royally.}

\brian{There is no $c_2$ for surfaces, only $c_1$. Also, you're missing a term in the Todd class. 
Should be
\ben
\pi_*( \ch(\cF) \Td(\cT_\pi)) = \pi_*( (1 + c_1(\cF) + \frac{1}{2}(c_1(\cF)^2) + \cdots)(1 +\frac{1}{2}c_1(\cT_\pi) + \frac{1}{12} c_1(\cT_\pi)^2))
\een
Thus, the degree two part is the pushforward of 
\ben
\frac{1}{12} c_1(\cT_\pi)^2 + \frac{1}{2} c_1(\cF) c_1(T_\pi) + \frac{1}{2} c_1(\cF)^2 .
\een  
That looks better? For $\cF = T_\pi^{\tensor n}$ we get $\frac{1 + 6n + 6n^2}{12} c_1(T_\pi)^2$. 
}
...
This space of deformations corresponds to continuous parameters we can vary in the action functional \owen{such as [now we spell out what they are]}. 
...
There is a spectral sequence \owen{you have to say what it is!} converging to the cohomology of the global observables $H^*\Obs^\q(\Sigma)$ with $E_2$ page given by the cohomology of the global observables of the $bc\beta \gamma$ system which we have already computed:
\bestar
...
\owen{Don't say "it is clear": just say why! It's not much more writing.}
