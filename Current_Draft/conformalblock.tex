...
which encodes the derived tangent space of the moduli of Riemann surfaces at $\Sigma$ and of holomorphic functions to~$V$. \owen{Eugene: At 0? (origin?)}
...
\owen{Eugene found this a bit quick.}
...
\een  
When $\cF = T^{\tensor n}_{\pi} [1]$ \owen{we should probably explain how shifts affect signs \dots}, the expression for the first Chern class is $-\frac{1 + 6n + 6n^2}{12} c_1(T_\pi)^2$.
When $\cF = \cO \tensor V$ we simply get~$\dim(V)$. 
\owen{Maybe do $n =1 $ case explicitly first and then explain general case. We also change notation from scripty T to straight T.}
...
\]
\owen{We are adding two contributions so maybe give steps carefully.}
...
\owen{No one knows what that means. We should add a few words.}
...
\een 
(That is, these vector spaces are the first \owen{Eugene: zeroth earlier?} cohomology group of the relevant deformation complex.)
...
\owen{Eugene suggests to move this before 6.2}
