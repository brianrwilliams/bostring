...
See, for instance, \cite{GSW1}, Chapter 2 of \cite{polchinski},
and~\cite{Scherk}. 
\end{rmk}
...
\owen{Eugene remarked "it's worth explaining what this means conceptually".}
\brian{I've explained what I meant by "compatible". Is that what he's referring to? Not sure what else to expand on...}
...
The operator $\Delta_{g_0+\alpha}$ refers to the Laplacian corresponding to the deformation of the metric $g_0 + \alpha$. 
We write this operator as an expansion in the field $\alpha$
\ben
\Delta_{g_0 + \alpha} = \sum_{n \geq 1} \frac{1}{n!} \int_{\Sigma} D_n(\alpha,\ldots, \alpha) .
\een
Hence, the $D_n$ are differential operators of the form 
$D_n : \left(\Sym^2(T_\Sigma)\right)^{\tensor n} \to {\rm Diff}^{\leq 2} (\Sigma)$ 
where ${\rm Diff}^{\leq 2}(\Sigma)$ are order $\leq 2$ differential operators.
Thus, for each section $\alpha$ of $\Sym^2(T_\Sigma)$, we get a second-order differential operator $D_n(\alpha,\cdots,\alpha)$ acting on functions on $\Sigma$. 
%(This term is the $n$th term in the Taylor expansion, so it is homogeneous of order $n$: $D_n(t \alpha) = t^n D_n(\alpha)$ for a scalar $t$.)
We write $S^{ghost}(\varphi, \alpha, X,f)$ for terms in the BV action functional that encodes how the ghosts act on the physical fields of the theory.  
It is of the form
\begin{align}
...
The second term encodes how vector fields and Weyl transformations act on the perturbed metric $g_0 + \alpha$ and the remaining terms are required to ensure the gauge symmetry is consistent (satisfies the classical master equation). 

Writing down an explicit formula for $D_n(\alpha,\ldots,\alpha)$ is a rather involved exercise (and not needed here).
