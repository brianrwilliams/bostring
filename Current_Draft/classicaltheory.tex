...
Thus, for each section $\alpha$ of $\Sym^2(T_\Sigma)$, we get a second-order differential operator $D_n(\alpha)$ acting on functions on $\Sigma$. (This term is the $n$th term in the Taylor expansion, so it is homogeneous of order $n$: $D_n(t \alpha) = t^n D_n(\alpha)$ for a scalar $t$.)
