...
\owen{It's not about the derivation stuff (which I've now suppressed) but about the interaction $I^q$. His complaint was that one might naively think that the $L \to 0$ limit is just the classical action. I think we mean something slightly more subtle. The mod $\hbar$ term is the classical action, I believe, but there are $\hbar^n$ terms from diagrams with loops. Thus this is {\em not} what you get under Kevin's correspondence. It's this thing where you construct a quantization by setting $I_t = I^{cl}$ and running RG flow, then you set $I[L] = \lim_{t \to 0} I_t[L]$. Right?}
