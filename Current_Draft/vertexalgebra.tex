...
That is, the weights of all Feynman diagrams are finite (compare to Proposition \ref{prop no CT}),
\item If the renormalized action $\{I[L]\}$ satisfies the quantum master equation then it determines a vertex algebra derivation $D_I$ of~$\cV_{\rm free}$ of the form
\ben
D_I = \oint I^q\, \d z
\een
that is square-zero and of cohomological degree one.
Here, $I^q = \lim_{L \to 0} I[L]$.
\owen{Eugene pointed out that this notation was kind of confusing. We should explain more carefully what $I[L]$ is and what the output $I^q$is.}
\brian{How's that? I've added a remark below about how the derivation acts on the vertex algebra}
...
\owen{I'm really not wild about evaluating the observables on a closed set like $C$, since it's not obviously well-behaved. Can we just use a small thickening?}
\brian{How's that?}
...
\owen{Eugene pointed out that the next paragraph is way too fast. It presupposes that someone understands that the factorization product is independent of the location of the disks, and what the E-H argument is! This is true of topologists, but probably not physicists or geometers!}
...
Hence, let $O,O'$ be closed observables on disks $D,D'$, respectively.
For simplicity, we assume that the closure of $D$ is strictly contained in $D'$. 
If $C = \partial D'$, and $N_C$ is a small tubular neighborhood (so an annulus), we can define $\int_C \Tilde{O}' \in \Obs^\q (N_C)$ as above. 
Finally, for $D''$ another disk containing $D$ and $C$, 
consider the factorization product $\mu : \Obs^\q(D) \tensor \Obs^\q (N_C) \to \Obs^\q(D'')$ and define
\ben
...
\owen{I'm not wild about using $\star$ for the factorization product, since it's not a single binary product.}
\brian{Not sure if what I did is any better.} 
