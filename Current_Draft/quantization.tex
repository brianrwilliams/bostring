...
(This process is a version of an obstruction theory in homological algebra.)
\end{enumerate}
...
&= \int_{z \in \CC} c(z)P_\ell^L(z,w)\big|_{z = w}  \\
&= \int_{z \in \CC} \int_{\ell}^L \d t f(z) \partial_z \left(\frac{1}{4\pi t} \frac{\zbar - \overline{w}}{2t} e^{-|z-w|^2/4t} (\d z - \d w) \right)\bigg|_{z = w}\, \d \zbar\\
&= \int_{z \in \CC} \int_{\ell}^L \d t f(z) \left(\frac{1}{4\pi t} \left(\frac{\zbar - \overline{w}}{2t}\right)^2 e^{-|z-w|^2/4t} (\d z - \d w) \right)\bigg|_{z = w}\, \d \zbar\\
&= 0,
\end{align*}
...
Mathematically, it defines a homotopy equivalence between the theory at scale $\ell$, defined using $S[\ell]$, and the theory at scale $S$, defined using~$S[L]$. 
\owen{Eugene: It might be good to expand on this a little.}
\brian{I'd prefer not to say any more. Seems like a rabbit hole, unless you have something concise you want to add.}
\end{rmk}

\owen{I'm not sure exactly what Eugene wants, but perhaps we can forward reference to something? I'm not sure this homotopy equivalence statement is true unless we satisfy the QME \dots}
...
\owen{Add picture.}
\brian{will do when I'm tired tonight.}
