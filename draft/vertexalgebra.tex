...
\begin{rmk}
For all examples of topological field theories arising from action functionals with which we are familiar (e.g., Chern-Simones theories),
...
Moreover, every $E_n$ algebra determines a fully-extended framed n-dimensional TFT in the functorial sense, albeit with values in an unusual target $(\infty,n)$-category, by work of Scheimbauer \cite{Scheim},
so that the holomorphic bosonic string does determine a functorial 2-dimensional TFT.
\end{rmk}
...
Our method is based on Witten's characterization of TFT in \cite{WittenTop}: 
a TFT is a theory whose the stress-energy tensor is (homotopy) trivial. 

Hence we want to show that the translation symmetries of the holomorphic bosonic string act trivially on the cohomology of the observables.
...
By Noether's theorem any symmetry of a theory determines observables: 
...
We'd like to point out an immediate corollary of the above analysis.
Consider the vector bundle (of infinite rank) on $\CC$ whose fiber over a point $x \in \CC$ is the observables $\Obs^\q_{\rm string} (D_x)$ where $D_x$ is any disk containing $x$. 
By local constancy, this is well-defined. 
We denote this vector bundle by $\sO{\rm bs}$. 
Since we have trivialized the holomorphic and anti-holomorphic translations up to homotopy, we can identify fibers up to quasi-isomorphism. 

\begin{cor} The trivializations $\eta,\Bar{\eta}$ equip $\sO{\rm bs}$ with a flat connection. 
...
To construct the bracket we will use the method of ``descent" for local observables. 
Expositions of this construction as related to two-dimensional gravity can be found in \cite{WittenDescent,Dijk}.
The basic idea is the following.
If $O$ is a local observable that is closed for the quantum differential, we can use the trivializations of translation to promote $O$ to a {\em non-local} observable supported on any closed submanifold of the Riemann surface. 

\begin{construction} We have just seen that the derivations $\frac{\d}{\d z}$ and $\frac{\d}{\d \Bar{z}}$ act trivially, up to homotopy, on the factorization algebra.
Given any observable $O$, the relations (\ref{d/dzbar}) and (\ref{d/dz}) allow us to define a differential form-valued observable $\Tilde{O}$ as follows. 
The 0-form part $\Tilde{O}^{0}$ is just the observable $O$. 
The 1-form part $\Tilde{O}^1$ is equal to the linear combination
\ben
...
This implies that given any closed observable $O$ and submanifold $C \subset \Sigma$, 
we can define a cocycle in the observables via
\ben
...
There is another way to view $\Tilde{O}$ coming from the fact that the observables combine to given a local system $\sO{\rm bs}$. 
Given any local system $\sL$ on $\CC$ we can consider its de Rham complex $\Omega^*(\CC, \sL)$.
The cohomology $H^0(\sO{\rm bs})$ is a trivial local system with fiber over $x$ given by $H^0({\rm Obs}^\q_{\rm string}(\CC))$.
Thus, there is a ``flat-section" map 
\ben
...
\begin{prop} 
The bracket $\{-,-\}_{\rm Ger}$ together with the product $\cdot$ determine the structure of a Gerstenhaber algebra on the subspace $V \subset H^*(\Obs^\q(D))$. 
...
To see that the brackets coincide, 
we note that the derivation $\eta$ that trivializes $\d / \d z$ agrees with Lian-Zuckerman's trivialization.
\end{proof}
