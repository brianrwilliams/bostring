...
\begin{claim} In the ``infinite volume limit" $h \to \infty$ the BV action above becomes
\ben
S_{Poly}^h \to S(\beta, \gamma, b,c) + \Bar{S}(\Bar{\beta}, \Bar{\gamma}, \Bar{b}, \Bar{c})
\een
...
\ben
\int_\Sigma {\rm ev}_V(B, \d \varphi) - \frac{1}{2} \int_\Sigma h^\vee(B, \star B)
\een
where: ${\rm ev}_V$ represents the evaluation pairing between $V$ and its dual, $\star$ is the Hodge star operator for the metric $g_0$, and $h^\vee$ denotes the dual metric on $V$. 
...
Furthermore, we can use the metric $g_0$ to define a projection map
\ben
\Sym^2(T\Sigma) \to \Omega^{0,1}(\Sigma ; T^{1,0}\Sigma) \oplus  ...
\een
