...
\brian{Confused by what you mean by ``does b ever appear"? But I've added further comments above about degrees.}
...
\owen{I meant that there's a process by which one can work out the BV action from the naive action plus the relevant gauge action.}
\brian{I agree, but even the $b$ fields appear in the usual string theory literature, so maybe their appearance here won't surprise people? I'm happy to sketch the construction of a BV theory from a naive gauge theory, but it may belong in the overview section, so I've added a bullet there.}
...
For instance, here we see $H^1(\Sigma_c,V)$, the cokernel of~$\dbar+c$. \owen{??? We should explain how we see relations of relations in this example}
\end{rmk}
...
\owen{I'm trying to add in the names for fields as well to make life easier}
\[
\begin{array}{ccccc}
...
& +  S'(X, f, \alpha) 
\end{align}
\brian{You used both ``begin{equation}'' and ''begin{align*}''. Latex was giving me an error, and I don't think you need to use both.}
\owen{I did that to have a single equation number for the equation. If you use align*, then you get no equation number. If you use align, as I did now, you get three. It's poopy.}
...
\owen{I'm not sure I like this description, although it's accurate. Maybe something more concrete like ``for each $\alpha$, we get a second-order differential operator $D_n(\alpha)$. (This term is the $n$th term in the Taylor expansion, so it homogeneous of order $n$: $D_n(t \alpha) = t^n D_n(\alpha)$ for a scalar $t$.)'' I also removed the $n$-fold appearance of $\alpha$.}
...
\owen{Would it be crazy to specify $S'$ to some extent?}

An explicit formula for $D_n(\alpha,\ldots,\alpha)$ is a rather involved exercise (and not needed here).
%can be derived from the power series expansion of the Laplace-Beltrami operator in terms of the metric. 
...
\owen{Is there some way to see this from the concrete computation of $D_1$ above?}
