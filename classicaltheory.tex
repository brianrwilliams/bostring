...
\owen{$b$ essentially appears as a Lagrange multiplier, so it doesn't have an ``intrinsic meaning'' itself. People might find it confusing since it is not in degree 0, and hence looks ``unphysical,'' plus it isn't obvious from the naive/usual approach to specifying field theories. (That's why I mentioned that it might be nice to include a pointer to Kevin's discussion of where BV action comes from.)}
...
The second line encodes how vector fields act on the maps of the $\sigma$ model \owen{I think we should hyphenate: $\sigma$-model}. 
...
\owen{I promoted this to a lemma. I think it's true, and we don't feel obligated elsewhere to give full proofs of every assertion.}
...
\owen{We could also just say ``the holomorphic string on $\Bar{\Sigma}$, which is the conjugate complex structure.''}
\end{rmk}
...
Notice that it is simply the action functional for the $\sigma$ model \owen{hyphenate} of maps from $(\Sigma, g_0)$ to $(V, h)$. 
