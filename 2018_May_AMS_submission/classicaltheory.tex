...
\begin{rmk}
For original physics references that study the holomorphic splitting of the Polyakov action (and more general conformal field theories) we refer to the sources \cite{Belavin, Laz}.
\end{rmk}
...
We now consider the remaining terms in the first line of the action. 
We work in holomorphic coordinates with the complex structure that the background metric $g_0$ determines. 
...
With respect to this splitting we will write sections as $\alpha = c^{0,1} + \Bar{c}^{1,0} + f g_0$, where $f \in C^\infty(\Sigma)$. 
By performing a Weyl rescaling, we see that the action functional only depends on $\mu, \Bar{\mu}$. 
This is essentially due to the fact that the ordinary Riemannian $\sigma$-model only depends on the conformal class, equivalently complex structure, of the Riemann surface. 
In the first-order formalism we introduced in the preceding paragraph, we find that the remaining terms in the first line of (\ref{polyakov bv}) read 
\ben
\int_\Sigma \<\beta , [c^{0,1}, \gamma]\>_V + \int_\Sigma \<\Bar{\beta} , [\Bar{c}^{1,0}, \bar{\gamma}]\>_V.
\een
