...
As we will see, it gives an approach to the method of descent, and another explicit description of the $E_2$ algebra that the quantum observables of the bosonic string define.

%First, we will see how to extract a local system (in the derived sense) from the observables on $\Sigma = \CC$. 
For this, we will look at an important class of observables that are {\em local}. 
We have already encountered local functionals in our description of the deformation complex in Section \ref{sec: moduli}, but we recall the main idea.

Let $J E_{\rm string}$ denote the $\infty$-jet bundle of the classical fields of the holomorphic bosonic string. 
This is a $D$-module on the Riemann surface, equipped with the usual flat Grothendieck connection $\nabla^{jet}$. 
The classical action functional equips functionals on the $\infty$-jet bundle
\ben
\left(\sO(J E_{\rm string}), \{S,-\}\right) = \left(\prod_{k \geq 0} \Sym^k(J E_{\rm string})^\vee, \{S,-\}\right)
\een
with the structure of a commutative dg algebra in $D$-modules.
...
%To define this, we will take advantage of the symmetry by the group $S^1$ on the quantum theory induced by rotations on $\CC$.

%Let $D_0 \subset \CC$ be a disk centered at the origin $0 \in \CC$.
%Then, the action of $S^1$ on the quantum theory induces one on the cochain complex of observables supported on $D$: $\Obs^{\q}_{\rm string}(D)$. 
%Let
%\ben
%\Obs^{\q}_{\rm string}(D_0)^{(k)} \subset \Obs^\q_{\rm string}(D_0)
%\een
%be the subcomplex consisting of those elements for which $\lambda \in S^1$ acts by multiplication by $\lambda^k$. 
%Taking the direct sum over all $k \in \ZZ$ we define the subcomplex
%\ben
%\sO{\rm bs}_0 = \bigoplus_{k \in \ZZ} \Obs^{\q}_{\rm string} (D_0)^{(k)} .
%\een
%
%In the classical limit, there is an isomorphism
%\ben
%\sO{\rm bs}_0 \mod \hbar \; \; \cong \; \; \left(\Sym((J_0 \sE_{\rm string})^\vee), \{S,-\}\right)
%\een
%where $J_0\sE_{\rm string}$ is the fiber at $0 \in \CC$ of the bundle of $\infty$-jets of the fields, and $S$ is the classical action functional.
...
This acts on fields, as well as the jets of fields in the obvious way.
...
\] 
A straightforward calculation shows that if $O$ is closed for the classical differential $\{S,-\}$
then 
\[
\Tilde{O} = \Tilde{O}^{0} + \Tilde{O}^1 + \Tilde{O}^2 
\]
satisfies $(\d_{dR} + \{S,-\}) \Tilde{O} = 0$.
\end{construction}
