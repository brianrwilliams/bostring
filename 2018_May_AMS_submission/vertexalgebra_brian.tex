...
\brian{I want to say that we can do this over each point to get a vector bundle whose fiber looks like jets at that point, so we inherits a natural D-module structure. 
Then, the calculation above says that the flat connection is actually trivializable, so that we can perform descent.}
