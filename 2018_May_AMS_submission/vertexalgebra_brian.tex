...
\owen{I did not understand the following paragraph, so I continue below as if it were not included. Please feel free to reincorporate this material as you see fit.}
...
We choose to encode this idea in the following formal terms.
Consider the de Rham complex of our $D$-module
\ben
\Omega^*(\CC , \sO(J E_{\rm string}))
\een
...
Integration along $C$ then determines a map of cochain complexes
\begin{equation*}
\label{eqn: int over C}
\int_C : \Omega^*(N_C , \sO(J E_{\rm string})) \to \Obs^{\cl}_{\rm string}(N_C) \tag{$\dagger$}.
\end{equation*}
\owen{We assert but don't prove that. Hope that's not an issue.}
...
In this case it is straightforward to show that the entire vertex algebra $\cV_{\rm string}$ is realized by the image of the map~(\ref{eqn: int over C}).
\owen{I really want the {\em classical} VA here. Do we have such a notation?}
...
Given any pointlike operator $O$, 
the relations (\ref{d/dzbar}) and (\ref{d/dz}) allow us to define a differential form-valued observable $\Tilde{O} \in \Omega^*(X, \sO(JE_{\rm string}))$ as follows. 
...
\Tilde{O}^{0} (z) = \tau_z O,
\een
which is continuous because we have a continuous action by translations.
Using the homotopies $\eta,\Bar{\eta}$, 
we define the 1-form part $\Tilde{O}^1$ as
\ben
...
\] 
By construction \owen{is this totally obvious?}, 
...
(\d_{dR} + \{S,-\}) \Tilde{O} = 0.
\]
We say $\Tilde{O}$ is {\em descended} from the pointlike operator~$O$.
\owen{OK terminology?}
\end{construction}

Combining the construction with the map~(\ref{eqn: int over C}), we find that a pointlike operator $O$ and a closed submanifold $C$ determine an observable
\ben
...
The main issue is that one cannot define a BV Laplacian on $\sO(J E_{\rm string})$
\owen{I'm not sure that's really true \dots} 
and so the map~(\ref{eqn: int over C}) does not extend immediately, as a cochain map, to the quantum observables.
\owen{It still make sense as a linear map, however, and by special features of our situation, these observables are enough to identify the Gerstenhaber bracket, etc.}

We now put these ideas to use.
...
\owen{The following paragraph no longer makes sense since we can't take such elements $O,O'$ as inputs.}
...
We then define
\ben
\{O,O'\}_{\rm Ger} := \mu \left(O, \int_C \Tilde{O}' \right) .
\een 
