...
We remark that every element of $\Gr \cV_{\rm string}$ can be realized by fixing any loop $C$ containing the fixed point (which we can take to be the origin) and applying descent to the pointlike operators.
...
\brian{I've commented out your construction, with explanation below. 
}

\owen{Let $O$ and $O'$ be two pointlike operators. 
...
\brian{The bracket only involves one circle. 
The factorization map we use is the annulus multiplying a disk to a disk, that's why it's a an operation on the vertex algebra.
The thing you should do is look at the two pointlikes $O,O'$ as you did above.
Think of $O$ as a disk observable (no descent) and consider 
\ben
\{[O], [O']\}_{Ger} = \mu(O, \int_C O') .
\een
We haven't checked that this is a bracket (graded antisymmetry, Jacobi,...) but I don't think we should do that now.
The thing that you did (which as you say has different cohomological degree) recovers the usual ''annular algebra" of the vertex algebra (which gives the Lie bracket of a Weyl type algebra). 
}

Let $O$ and $O'$ be two pointlike operators supported at $0 \in \CC$. 
Fix a disk $D$ centered at zero and a loop $C$ that wraps around $D$. 
Let $N_{C}$ be a tubular neighborhood of $C$ that does not intersect $D$. 

We can view $O$ as an element in $\Obs^\q(D)$.
Further, via descent we have the observable $\int_C \Tilde{O}'$ in $\Obs^\q(N_C)$. 
We suppose that they are cocycles, which lets us identify them with elements $[O]$ and $[O']$ of the vertex algebra.


If $D'$ is a disk centered at zero that countains the neighborhood $N_C$, let 
\[
\mu : \Obs^\q(D) \tensor \Obs^\q (N_{C}) \to \Obs^\q(D')
\] 
denote the factorization product corresponding to the configurartion $D \sqcup N_C \hookrightarrow D'$. 
We define a bracket by 
\ben
\{[O],[O']\}_{\rm Ger} := \mu \left(O , \int_{C} \Tilde{O}' \right) .
\een 
...
\brian{the degrees should be OK now}
