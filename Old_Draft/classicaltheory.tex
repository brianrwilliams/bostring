...
This paper restricts to the case where the target manifold is a vector space, 
rather than a Riemannian manifold with interesting topology or geometry.
Hence, let $V$ denote a complex vector space (the target),
...
The degree zero $c$ field is a smooth $(0,1)$-form with values in vector fields ``in the holomorphic direction,''
...
\end{equation}
(We discuss below how to think about fields with nonzero cohomological degrees as inputs to the action functional.)
...
The other equations then amount to solving for holomorphic sections (of the relevant bundle) with respect to this deformed complex structure.
For instance, the equation in $\gamma$ picks out holomorphic maps from $\Sigma$,
with the $c$-deformed complex structure, to~$V$.

\owen{Tell me what you think.}
...
This viewpoint of arriving at the bosonic string as a deformation of a free CFT is central to the analysis of the string in the physics literature. 
See, for instance, Chapter 3 of \cite{GSW1}, Chapter 2 of \cite{polchinski},
or~\cite{Scherk}. 
\end{rmk}
...
except when we specialize to the deformation-theoretic situation that is our main arena (i.e., the perturbative setting).
...
This construction determines a bundle $\Maps_{\dbar}(-,V)$ over $\cM$ by varying~$\Sigma$.
...
The observables for the classical theory are, in essence, the {\em co}\/sheaf of commutative algebras~$\cO(\Maps_{\dbar}(-,V))$,
and hence provide a factorization algebra.
...
and the second is the usual elliptic complex encoding holomorphic maps.
...
its action is usually the Taylor expansion of the nonperturbative action around a solution to the equations of motion,
and so the quadratic term---the underlying free theory---is essentially always the tangent complex of a nice derived stack.
...
That is, the action of infinitesimal diffeomorphisms and Weyl rescalings combine to give an action of the semi-direct product of vector fields with functions.  
...
\be \label{polyakov bv}
S^{BV}_{Poly} (\varphi, \alpha, X, f) =  \int_\Sigma h(\varphi, \Delta_{g_0 + \alpha} \varphi)\, \dvol_{g_0} +  S^{ghost}(X, f, \alpha) 
\ee
The operator $\Delta_{g_0+\alpha}$ refers to the Laplacian for the deformed metric $g_0 + \alpha$. 
We can expand this operator in terms of the $\alpha$ field:
\ben
\Delta_{g_0 + \alpha} = \sum_{n \geq 1} \frac{1}{n!} D_n(\alpha,\ldots, \alpha),
\een
where $D_n$ denotes a map 
\[
D_n : \left(\Sym^2(T_\Sigma)\right)^{\tensor n} \to {\rm Diff}^{\leq 2} (\Sigma),
\]
with ${\rm Diff}^{\leq 2}(\Sigma)$ denoting the differential operators of order~$\leq 2$.
In other words, for each section $\alpha$ of $\Sym^2(T_\Sigma)$, 
we get a second-order differential operator $D_n(\alpha,\cdots,\alpha)$ acting on functions $\phi$ on~$\Sigma$. 

We write $S^{ghost}(\varphi, \alpha, X,f)$ for the terms in the BV action functional that encode how the ghosts act on the physical fields of the theory.  
It has the form
\begin{align}
S^{ghost}(f,X,\alpha) & = \int_{\Sigma} h(\varphi, X \cdot \varphi) \dvol_{g_0} +  \int_\Sigma \<\alpha^\vee, L_X (g_0 + \alpha) + f (g_0 + \alpha)\> \\ & + \int_\Sigma \<X^\vee, [X,X]\> + \int_\Sigma \<f^\vee, X \cdot f\> .
\end{align}
The first term describes how vector fields act on the maps of the $\sigma$-model. 
The second term encodes how vector fields and Weyl transformations act on the perturbed metric $g_0 + \alpha$ and the remaining terms are required to ensure the gauge symmetry is consistent ({\em aka} that the whole action satisfies the classical master equation). 

Writing down an explicit formula for $D_n(\alpha,\ldots,\alpha)$ is a rather involved exercise,
and we do not need such a formula here.
The first case is illustrative.
Let us work locally on $\Sigma = \RR^2$ with $g_0$ the flat metric.
Then the operator $D_1(\alpha)$ is the sum of a first-order and a second-order differential operator
\ben
...
We can consider scaling it: for $t \in (0,\infty)$, replace $h$ by $t h$.
The ``infinite volume limit'' of $t \to \infty$ admits a nice description,
provided one rewrites the action functional in a first-order formalism.
By this, we mean that we want an action involving only first-order differential operators 
but which encodes an equivalent classical field theory to that encoded by the initial, second-order action.
In practice, one adjoins auxiliary fields to reduce the order of all differential operators in the action,
much as one extracts Hamiltonian mechanics from Newton's equation.
...
It is shown in the Appendix of \cite{GGW} how to make sense of the infinite volume limit of this usual $\sigma$-model. 
...
in the infinite volume limit $t \to \infty$, 
...
Of the remaining terms in the part of the action (involving just the physical fields), 
only the $D_1$ term survives in this infinite volume limit. 
\owen{Ref suggests we indicate why.}
This term, $\int_\Sigma h(\varphi, D_1(\alpha) \varphi)$, becomes
\ben
\int_\Sigma \<\beta , [c, \gamma]\>_V + \int_\Sigma \<\Bar{\beta} , [\Bar{c}, \bar{\gamma}]\>_V.
\een
where $c$ and $\Bar{c}$ are the $\Omega^{0,1}(\Sigma ; T^{1,0}_\Sigma)$ and $\Omega^{1,0}(\Sigma ; T^{0,1}_\Sigma)$ components of $\alpha$ with respect to the embedding
\ben
\Omega^{0,1}(\Sigma ; T^{1,0}_\Sigma) \oplus \Omega^{1,0}(\Sigma ; T^{0,1}_\Sigma) \hookrightarrow \Sym^2(T_\Sigma)
\een
defined by the fixed metric $g_0$. 
Note that the off-diagonal terms $\Omega^{0,1}(\Sigma ; T^{0,1}_\Sigma)$ and $\Omega^{1,0}(\Sigma ; T^{1,0}_\Sigma)$ do not appear which reflects the fact that the theory is conformal. 

This first term above is precisely the third term in the holomorphic string action functional~(\ref{bosaction}), which describes how deformations of complex structure couple to the fields of the $\sigma$-model. 
