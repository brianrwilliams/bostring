...
\owen{For some reason this is irrelevant.}
...
\owen{Is there a slick way to see that $F[L] = G[L]$?}
\brian{We want $F[L] = 13 G[L]$, right? The only way I know how to do this is to actually write out the integrals. That's what I'll do below, and you can tell me if it's too long.}
The interested reader can verify that $F[L]$ is not just zero, 
so that the obstruction is nontrivial when $\dim_\CC(V) \neq 13$.
\end{proof}

\brian{Any direct proof I know for showing that the anomaly reduces to a one-loop calculation reduces to an argument in Si's B-twist paper or Kevin's Witten genus.}

\begin{proof} \brian{Here I've already assumed we've reduced the calculation to two wheel diagrams: A) with internal edges labeled by the $bc$ heat kernel and propagator, respectively. B) with internal edges labeled by the $\beta\gamma$ heat kernel and propagator, respecitvely. }
...
\een 
\brian{Is this too much detail, or not enough? I'll wait for your feedback before going further, but the last two integrals are very similar.}

\end{proof}

\brian{***BELOW MAY GO IN APPENDIX***}
