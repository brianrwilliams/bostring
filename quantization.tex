...
\begin{prop}
For any graph $\Gamma \in {\bf Graph}_{\rm string}$ allowed by the combinatorics of the string action functional,
there is a well-defined limit
\[
\lim_{\ell \to 0} w_{\Gamma}(P_{\ell}^L,I_{\rm string})
\]
for any $L > 0$.
\end{prop}
...
In this case \owen{and now I think I'll just write an explicit integral for a 2-vertex or 3-vertex wheel and assert that it is not divergent by explaining loosely the argument.}
\end{proof}
