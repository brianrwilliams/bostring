...
\brian{I apologize, but it doesn't look like my super naive estimate is going to work. Here's what does work. Also, it's a two real dimensional integral so the correct variance is $t_1t_2 / (t_1+t_2)$ (the square of what you use). 
The $t$-integral you get out is
\ben
\int_{t_1,t_2 = \ell}^L \frac{(t_1 t_2)^2}{(t_1+t_2)^5} \d t_1 \d t_2  .
\een
Now, by AM-GM $|t_1 + t_2| \geq C \sqrt{(t_1 t_2)}$ for some $C > 0$. Thus, the integral is bounded by 
\ben
1/C \int_{t_1,t_2 = \ell}^L \frac{(t_1 t_2)^2}{(t_1t_2)^{5/2}} \d t_1 \d t_2 = \int_{t_1,t_2 = \ell}^L \frac{1}{\sqrt{t_1 t_2}} \d t_1 \d t_2
\een
which does have a finite $\epsilon \to 0$ limit. Look OK?
}

\owen{I'm going to try again. That's a good point about the behavior of variance as a function of dimension, but we're actually taking an integral over $\CC^2$. I tried using the AM-GM inequality earlier but it didn't help with my fucked up exponents. Time to try again.}

\brian{Actually, we are only doing an integral over $\CC$. We are only applying Wick's Lemma for the integral over $u \in \CC$. The final answer is written as an integral over $v = z_2$ of some differential operator applied to the compactly supported function $f_1f_2$. So, in this case the final answer is proportional to $\int_{v \in \CC} \partial^4 f_1(v) f_2(v)$.
}
