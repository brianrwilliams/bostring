...
So, open by open, the factorization algebra assigns the same vector space for the deformed theory.\brian{should I keep going?}
\end{rmk}

\owen{We need to figure out how such a chiral deformation affects the vertex algebra in more explicit terms. 
For instance, do we just use $e^{I/\hbar}$ as the vacuum vector and generate a new VOA? Or do we ``conjugate'' the factorization structure maps by $e^{I/\hbar}$ to get a Wick-type formula?}

\brian{I think I want to stress the following. Si's result implies that we get an interesting dg vertex algebra. Forgetting the differential we see the same underlying graded vertex algebra as the undeformed theory. Indeed, what we do is applying your and Kevin's result for the underlying graded vertex algebra. Then, using the language from our CDO paper we see that the differential $\{I,-\}$ satisfies the requirements to determine a derivation of the graded vertex algebra. This is the dg vertex algebra giving semi infinite cohomology. This avoids saying anything about a Wick-type formula.}
...
The classical observables are simply functions on the solutions to the classical equations of motion, so the Poisson structure is completely natural.
...
\brian{Possibly state the relationship to semi-infinite cohomology}
\owen{Yes we should state it, probably as a remark.}
