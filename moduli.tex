...
For instance, the jets at the origin of the Dolbeault complex $\Omega^{0,*}(\CC ; V)$ are
$(V [[z,\zbar]] [\d \zbar] , \dbar)$,
where $\dbar$ is the formal Dolbeault differential. 
An example of an element is $\widehat{\gamma} = \sum_{m,n} \frac{1}{m!n!}g_{mn} z^m \zbar^n$,
...
\]
We will the first kind of term a {\em chiral} interaction, as it only depends on holomorphic derivatives.
...
it is simply saying that for a solution $\gamma$ to the equation of motion $\dbar \gamma = 0$, 
...
that are polynomials built out of continuous linear functionals of the jet.
This choice is the standard and natural one for variational problems.
We note as well that we should exclude constant functionals,
...
and the Lie algebra of formal vector fields ${\rm W}_1 = \CC[[z]]\partial_z$.
...
are thus quasi-isomorphic to $\cred({\rm W}_1)$,
...
\subsubsection{Interaction terms that appear at one loop}
...
{The $\CC^\times_{\rm cot}$-weight zero piece}
