...
\owen{We had a conflict, so I just disentangled stuff without editing. Let's make sure our edits merge nicely.}
...
Thus, for quantum corrections at one loop we consider local functionals that are of weight zero for this $\CC^\times_{\rm cot}$-action.\brian{I want to cite BV laplacian and quantum differential here}
Put simply, these are local functionals that only depend on the base of the shifted cotangent bundle. 
On a Riemann surface $\Sigma$, 
the $\CC^\times_{\rm cot}$ weight $0$ \owen{I think this is the only weight we ever use, so you can drop the precursor.} local functionals are of the form
\ben
...
\begin{prop} 
There are $\GL(V)$-equivariant quasi-isomorphisms of the translation invariant deformations
\begin{itemize}
\item[(0)] of weight $0$:
\ben
...
We have already mentioned that we want to study Lagrangian densities, which are functions on jets of fields. 
