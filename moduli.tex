...
\owen{I inserted a description because I don't think $W_1$ is obvious notation.}
...
\[
\Def_{\rm string}(\CC,V)^\CC \simeq \cred^*(\wone, \Sym(V^\vee[z^\vee] \oplus V[z^\vee] \d z^\vee \oplus W_1^{\rm ad}[2])) 
\]
...
\owen{Please double check that $b$ field part.}
...
\[
\Def_{\rm string}(\CC)^{\CC, (1)} \simeq \Sym(V^*) \tensor V[1]
\]
...
\owen{I think we might be able to just to sketch the key argument in light of the discussion in the preceding subsection. That is, just point out the weights argument. We can also just say that it is proved by arguments that are similar but easier than what we do in the next subsection.}

\owen{We should make a small comment about why we take into account $GL(V)$ equivariance,
such as that we want to take into account the other natural symmetries of the target $V$.}
...
There is a $\GL(V)$-equivariant quasi-isomorphism
\[
\Def_{\rm string}(\CC)^{\CC, (0)} \simeq \CC[-1] \oplus \Omega^2_{cl}(V)[1] \oplus \Omega^1(V) \oplus \Omega^1_{cl}(V)[-1] 
\]
...
The second term is a choice of \owen{potential?} on the target and contributes a term \owen{???} to the action.
...
\owen{One-forms is hyphenated. The need to do this is clear from considering "two forms" which is ambiguous without the hyphen.}
...
The Euler vector field $L_0 = z \partial_z$ induces a grading on $\wone$ and hence on the Chevalley-Eilenberg complex of $\wone$ with coefficients in any module. 
...
Since $V$ is flat,
we will see that all such \owen{what does 'such' refer to?} deformations will be trivializable. 
